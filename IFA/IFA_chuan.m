...
%function[lamBak,etaBak]=IFA_chuan(strcat(inputDir),strcat(inputFile),kinit,n,p,strcat(outputDir))

inputDir='inputDirToReplace';
inputFile='inputFileToReplace';
outputDir='outputDirToReplace';
kinit=kinitToReplace;
n=nToReplace;
p=pToReplace;
...
file=strcat(inputDir,'/',inputFile,'.txt')
...
nrun=2000; burn=1000; thin=1; sp =(nrun - burn)/thin; % number of posterior samples

%kinit = 10;                 % number of factors to start with
b0 = 1; b1 = 0.0005;
...
for g = 1:rep
    ...
    like_v=zeros(1,(nrun-burn)/100+1);
    like_v(1)=-Inf;
    ...
    for i=1:nrun
        ...
        %%%%%%%%%% Chuan's implementation, select the parameters based on the likelihood.
        
        if mod(i,100) == 0 & i>5000
            like=0;
            for  j=1:p
                like_tmp = normpdf(Ytil(:,j),0,sqrt(Sigma(j,j)));
                like = like + sum(log(like_tmp(like_tmp~=0)));         
            end
            like_tmp=gampdf(diag(Sigma),as,1/bs);
            like = like + sum(log(like_tmp(like_tmp~=0)));   
            
            like_tmp=normpdf(eta,0,1);
            like = like + sum(sum(log(like_tmp(like_tmp~=0))));
            
            like_tmp=gampdf(psijh,df/2,2/df);
            like=like+sum(sum(log(like_tmp(like_tmp~=0))));
            
            like_tmp=gampdf(delta(1),ad,1/bd);
            like=like+sum(sum(log(like_tmp(like_tmp~=0))));
            
            like_tmp=gampdf(delta(2:k),ad2,1/bd2);
            like=like+sum(sum(log(like_tmp(like_tmp~=0))));
            
            like_tmp=normpdf(Lambda,0,sqrt(1./Plam));
            
            like=like + sum(sum(log(like_tmp(like_tmp~=0))));        

            count=count+1;
            like_v(count)=like;
            if(like_v(count)>like_v(count-1))
                lamBak=Lambda;
                etaBak=eta;
            end
        end
...
%dlmwrite(strcat(outputDir,'/',inputFile,'_lam_ifa.txt'), lamBak, 'delimiter', '\t','precision', 6);
%dlmwrite(strcat(outputDir,'/',inputFile,'_ex_ifa.txt'), etaBak, 'delimiter', '\t','precision', 6);

dlmwrite(strcat(outputDir,'/',inputFile,'_lam_ifa.txt'), Lambda, 'delimiter', '\t','precision', 6);
dlmwrite(strcat(outputDir,'/',inputFile,'_ex_ifa.txt'), eta, 'delimiter', '\t','precision', 6);
