...
SFAmix <- function(Y_TMP_param,nrow_param, ncol_param, a_param,b_param, nf_param, itr_param, LAM_out, EX_out, Z_out, O_out, EXX_out, PSI_out, nf_out, out_itr, out_dir,rsd, lam_method, x_method, tol){
    ...
    result <- .C ("SFAmix",
                  as.double(Y_TMP_param),as.integer(nrow_param), as.integer(ncol_param), as.double(a_param),as.double(b_param), as.integer(nf_param), as.integer(itr_param), LAM=as.double(LAM_out), EX=as.double(EX_out), Z=as.double(Z_out), O=as.double(O_out),EXX=as.double(EXX_out),PSI=as.double(PSI_out), nf=as.integer(nf_out), as.integer(out_itr), as.character(out_dir),as.integer(rsd),as.character(lam_method), as.character(x_method), as.double(tol),PACKAGE="SFAmix")
...
#' result = SFAmixR(data$y,nf=100,a=0.5,b=0.5,itr=5000,out_dir="results",tol=1e-5,x_method="sparse",rsd=123)
...
SFAmixR <- function(y=NULL,nf=100,a=0.5,b=0.5,itr=5001,rsd=NULL,out_itr=500,out_dir=NULL, lam_method=NULL, x_method=NULL, tol=NULL, qnorm = TRUE){
   ...
   if(is.null(y)){
    ...
    result <- SFAmix(y,sn,dy,a,b,nf,itr,LAM_out,EX_out,Z_out,O_out,EXX_out, PSI_out, nf_out, out_itr, out_dir2, rsd, lam_method, x_method, tol)
    return(result)
}
...
gen_SFAmix_data <- function(std=2){
    nf.s <- 20
    nf.d <- 10
